...
export default function NewProject() {
  ...
  const [form, setForm] = useState({
    ...
    cover_image: '', images: [] as string[],
    featured: false, render_type: 'photography', status: 'draft',
  })
  ...
  return (
    <div style={{ maxWidth: '800px' }}>
      <div style={{ marginBottom: '40px' }}>
        <p style={{ fontFamily: 'var(--font-inter)', fontSize: '9px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--brass)', marginBottom: '6px' }}>Projects</p>
        <h1 style={{ fontFamily: 'var(--font-cormorant)', fontSize: 'clamp(28px,5vw,40px)', fontWeight: 300, color: 'var(--offwhite)' }}>New Project</h1>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px,1fr))', gap: '24px' }}>
          <div><label style={labelStyle}>Title *</label><input style={inputStyle} value={form.title} onChange={e => handleTitleChange(e.target.value)} placeholder="Project title" /></div>
          <div><label style={labelStyle}>Slug *</label><input style={inputStyle} value={form.slug} onChange={e => set('slug', e.target.value)} placeholder="project-slug" /></div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px,1fr))', gap: '24px' }}>
          ...
          <div><label style={labelStyle}>Location</label><input style={inputStyle} value={form.location} onChange={e => set('location', e.target.value)} placeholder="Lagos, Nigeria" /></div>
        ...
        <div>
          ...
          <textarea style={{ ...inputStyle, resize: 'none', lineHeight: 1.8 }} rows={5} value={form.description} onChange={e => set('description', e.target.value)} placeholder="Describe the project..." />
        </div>
        ...
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', paddingTop: '16px', borderTop: '1px solid rgba(184,150,62,0.1)' }}>
          <button onClick={handleSave} disabled={saving} className="btn-brass-filled" style={{ fontSize: '10px', opacity: saving ? 0.6 : 1 }}>{saving ? 'Saving...' : 'Save Project'}</button>
