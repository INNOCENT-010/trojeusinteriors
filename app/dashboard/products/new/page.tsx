...
export default function NewProduct() {
  ...
  const [form, setForm] = useState({ title: '', slug: '', category: 'bedframe', description: '', images: [] as string[], featured: false, status: 'draft' })
  ...
  return (
    <div style={{ maxWidth: '800px' }}>
      <div style={{ marginBottom: '40px' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-cormorant)', fontSize: '40px', fontWeight: 300, color: 'var(--offwhite)' }}>New Product</h1>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
        ...
        <div><label style={labelStyle}>Description</label><textarea style={{ ...inputStyle, resize: 'none', lineHeight: 1.8 }} rows={4} value={form.description} onChange={(e) => set('description', e.target.value)} placeholder="Describe this piece..." /></div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', marginBottom: '16px' }}>
            {form.images.map((img, i) => (
              <div key={i} style={{ position: 'relative' }}>
                ...
                <button onClick={() => set('images', form.images.filter((_, idx) => idx !== i))} style={{ position: 'absolute', top: '4px', right: '4px', background: 'rgba(0,0,0,0.7)', border: 'none', color: '#fff', cursor: 'pointer', fontSize: '10px', width: '18px', height: '18px' }}>×</button>
              </div>
            ))}
          </div>
